						...
						//Check if block has enough confirms
							...
							if($enoughConfirms >= 120){
								//Count all the shares this username has, reward them in there account balance, then mark all the shares as counted
									...
									//Update stale/rejected shares to counted
										mysql_query("UPDATE `shares_history` SET `shareCounted` = '1' WHERE `shareCounted` = '0' AND `blockNumber` = '".$block["blockNumber"]."' AND `username` = '".$waitingUsername["username"]."' AND `our_result` = 'N'") or die(mysql_error());
